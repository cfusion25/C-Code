...
void Student::enroll(const map<int, Course>& course_db, int code)
{
	...
	if (course_db.find(code) != course_db.end()){
		...
		for (int i = 0; i < course_db.find(code)->second.get_num_prerequisites(); i++) {
			if (course_history.find(course_db.find(code)->second.get_prerequisites(i))==course_history.end()) {
				x++;
				break;
			}
		}
...
void Student::select_by_code(int base)
{
    ...
    while (!course_plan->iterator_end()) {
        Course c = course_plan->iterator_next();
    	if (c.get_code() > base) {
        	cout << c << endl;
    	}
    }
...
void Student::check_course(int code) const
{
	...
	if (course_plan->search(code)){
		course_plan->search(code)->print_value();
	}
	else {
		cout << "No" << endl;
	}
